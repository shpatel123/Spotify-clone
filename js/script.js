...
async function displayAlbums() {
    ...
    for (let index = 0; index < array.length; index++) {
        ...
        // Extract the folder name from the href and ensure it’s a subdirectory
        let folder = e.href.split("/").slice(-1)[0];
        if (folder && folder !== "songs") {  // Skip if it's the main 'songs' folder
            let infoJsonUrl = `http://127.0.0.1:5500/songs/${folder}/info.json`;
            console.log("Fetching:", infoJsonUrl);

            try {
                let a = await fetch(infoJsonUrl);
